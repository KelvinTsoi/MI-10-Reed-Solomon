...
int decode_rs_char(RS *rs, unsigned char *data, int *eras_pos, int no_eras)
{
...

    {
        ...
        int i, j, r, k;
        int nrootLength = NROOTS + 1;
        data_t u, q, tmp, num1, num2, den, discr_r;
#if 0
        data_t *lambda = new data_t[nrootLength];
        data_t *s = new data_t[nrootLength];

        data_t* b = new data_t[nrootLength];
        data_t* t = new data_t[nrootLength];
        data_t* omega = new data_t[nrootLength];

        data_t* root = new data_t[nrootLength];
        data_t* reg = new data_t[nrootLength];
        data_t* loc = new data_t[nrootLength];
#endif
        ...
        for (j = 1; j < NN - PAD; j++)
        {
            for (i = 0; i < NROOTS; i++)
            {
                ...
                {
#if 0
                    /* Array size limitation case of segmentation fault*/
                    if(j < 0 || j > 40)
                    {
